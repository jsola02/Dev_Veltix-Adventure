...
import CardCategoryItem from '/components/Blog/CardCategoryItem.vue';

const { getCatElixir, currentPage, getCategoryIdBySlug } = useWpApi();
const posts = ref<any[]>([]);
const selectedCategory = ref<number | undefined>(undefined);

// Obtiene los últimos 4 posts de la categoría "elixir" al montar el componente
onMounted(async () => {
  const newsCategoryId = await getCategoryIdBySlug('elixir');
