...
const getImageSrcSet = (media: any) => {
  ...
  return Object.keys(sizes).map(key => `${sizes[key].source_url} ${sizes[key].width}w`).join(', ');
};
</script>
    ...
    <article v-if="post" class="section_main_news-detail_article">
      ...
      <div class="section_main_news-detail_article_tumbnail">
        <img 
          ...
          :alt="post._embedded['wp:featuredmedia'][0]?.alt_text || 'Imagen destacada del post'"
      ...
      <div class="section_main_news-detail_article_content">
        ...
        <div v-html="post.content.rendered" class="section_main_news-detail_article_content_resume"></div>
      </div>
    </article>
  </section>
</template>
