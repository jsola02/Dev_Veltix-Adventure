<script lang="ts" setup>
  ...
  const { getPosts, getCategory, currentPage, totalPages, nextPage, prevPage } =
    useWpApi();
  ...
  <div>
    ...
    <section class="section_news_category">
      <div class="section_news_category_title">
        <h1 class="section_news_category_title_name"
          >Informacion sobre:
          <span
            class="section_headerblog_content_breadcrumbs_item_link-title"
            >{{ categoryName }}</span
          ></h1
        >
        <p class="section_news_category_title_description"
          >mantente informado para mejorarar tu experiencia de juego.</p
        >
      </div>

      <BlogCardNewsCategory
        class="section_news_category_article"
        v-for="post in posts"
        :key="post.id"
      ...
      <div class="post_pagination">
        <button
          class="post_pagination_button"
          @click="prevPage"
          :disabled="currentPage === 1"
          >Anterior</button
        >
        <span class="post_pagination_text"
